-- =================================================================
-- Migration: Add GMV Direct & Indirect columns and metrics aggregation
-- Purpose: Add gmv_direct and gmv_indirect fields to video_period_metrics table,
--          recreate video_with_metrics view, and update RPC functions
--          to aggregate and expose these new fields alongside product clicks
--          and impressions for reporting.
-- =================================================================

-- 1. Add columns to video_period_metrics
ALTER TABLE video_period_metrics ADD COLUMN IF NOT EXISTS gmv_direct NUMERIC DEFAULT 0;
ALTER TABLE video_period_metrics ADD COLUMN IF NOT EXISTS gmv_indirect NUMERIC DEFAULT 0;
-- product_clicks is referenced by get_videos_summary_for_period (total_clicks) but was
-- never present on the table; add it so the summary RPC and CTR/CR scorecards work.
ALTER TABLE video_period_metrics ADD COLUMN IF NOT EXISTS product_clicks BIGINT DEFAULT 0;

-- 2. Recreate video_with_metrics view
-- DROP first: new gmv_direct/gmv_indirect columns are inserted mid-list, which
-- CREATE OR REPLACE VIEW cannot do (it only allows appending columns).
DROP VIEW IF EXISTS video_with_metrics;
CREATE VIEW video_with_metrics AS
SELECT
  v.id,
  v.video_id,
  v.source_type,
  ...
  COALESCE(SUM(m.gmv_direct), 0)::NUMERIC AS gmv_direct,
  COALESCE(SUM(m.gmv_indirect), 0)::NUMERIC AS gmv_indirect,
...
-- 3. Recreate get_videos_with_period_metrics RPC
-- DROP first: return type changes (adds gmv_direct/gmv_indirect columns).
DROP FUNCTION IF EXISTS get_videos_with_period_metrics(date,date,text,uuid,numeric,bigint,text,text,boolean,integer,integer,uuid[],uuid);
...
BEGIN
  ...
  SELECT
    ...
    f.views, f.likes, f.comments, f.shares, f.orders, f.gmv, f.gmv_direct, f.gmv_indirect,
...
-- 4. Recreate get_videos_summary_for_period RPC
-- DROP first: return type changes (adds total_gmv_direct/indirect/clicks/impressions).
DROP FUNCTION IF EXISTS get_videos_summary_for_period(date,date,text,uuid,numeric,bigint,text,uuid[],uuid);
...
BEGIN
  ...
  WITH per_video AS (
    SELECT
      ...
      COALESCE(SUM(m.gmv_direct), 0)::NUMERIC AS gmv_direct,
      COALESCE(SUM(m.gmv_indirect), 0)::NUMERIC AS gmv_indirect,
      COALESCE(SUM(m.product_clicks), 0)::BIGINT AS product_clicks,
      COALESCE(SUM(m.impressions), 0)::BIGINT AS impressions
    FROM videos v
    LEFT JOIN video_period_metrics m
      ON v.video_id = m.video_id
  ...
  SELECT
    ...
    COALESCE(SUM(gmv_direct), 0)::NUMERIC   AS total_gmv_direct,
    COALESCE(SUM(gmv_indirect), 0)::NUMERIC AS total_gmv_indirect,
    COALESCE(SUM(product_clicks), 0)::BIGINT AS total_clicks,
