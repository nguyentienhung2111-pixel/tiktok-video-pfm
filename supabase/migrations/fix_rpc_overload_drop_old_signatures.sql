...
-- Background: add_assigned_user_filter_to_rpcs.sql added a new
--   parameter (p_assigned_user_id) using CREATE OR REPLACE. In
--   Postgres, changing the parameter list creates a *new* overload
--   instead of replacing the existing function. PostgREST then sees
--   two candidates with identical names and refuses to dispatch.
-- Fix: explicitly DROP the previous signatures, then recreate the
--   functions with the full signature including p_assigned_user_id.
-- =================================================================

DROP FUNCTION IF EXISTS get_videos_with_period_metrics(
  DATE, DATE, TEXT, UUID, NUMERIC, BIGINT, TEXT, TEXT, BOOLEAN, INT, INT, UUID[]
);
DROP FUNCTION IF EXISTS get_videos_summary_for_period(
  DATE, DATE, TEXT, UUID, NUMERIC, BIGINT, TEXT, UUID[]
);
